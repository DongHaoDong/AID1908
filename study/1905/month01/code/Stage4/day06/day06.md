...
```
# 整理思路
1. 爬取一级页面，提取所需数据+连接，继续跟进
2. 爬取二级页面，提取所需数据+连接，继续跟进
3. ... ...
# 代码实现思路
1. 所有数据最终都会在一级页面遍历每条数据时全部拿到
2. 避免重复代码 - 请求、解析所需定义函数
```
# Day06笔记
## 电影天堂二级页面抓取案例
- **1、领取任务**
...
```
# 思路
# 1. MySQL中新建表 request_finger,存储所有爬取过的连接的指纹
# 2. 在爬取之前，先判断该指纹是否爬过，如果爬取过，则不再继续爬取
```
**练习代码实现**
```
# 建库建表
...
# 方法二
    右键管理员进出入cmd命令行:pip install requests 
```
### requests.get()
* 作用
```
...
* 练习
```
1. 猫眼电影案例改写为requests模块实现
2. 将电影天堂案例改写为requests模块实现
...
**在线安装**
```
1. 安装插件:谷歌访问助手 - GoogleAccessHelper
2. 重启浏览器
3. 右上角 - 访问谷歌商店 - 搜索插件 - 添加至chorme
4. 重启浏览器
```
### 需要安装插件
```
1. Xpath Helper:轻松获取HTML的xPath路径
    # 开启/关闭:Ctrl + shift + x
...
### 定义
```
XPath即XML路径语言，它是一种用来确定XML文档中某部分位置的语言，同样适用于HTML文档的检索
```
### 示例
```html
<ul class="CardList">
...
//li/p[@class="name"]
3. 找比亚迪车的信息:获取ul节点下第二个li节点的汽车信息
//ul/li[2]
...
2. @ :获取属性值
    ...
    # 使用场景2(直接获取属性值)
    //div[@class="move-item-info"]/p/a/@title
```
### 匹配多路径(或)
```
