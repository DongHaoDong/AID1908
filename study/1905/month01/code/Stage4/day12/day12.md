...
node.send_keys(keyus.SPACE)
node.send_keys(keyus.CONTROL,'a')
node.send_keys(keyus.CONTROL,'c')
node.send_keys(keyus.CONTROL,'v')
node.send_keys(keyus.ENTER)
# 2. 鼠标操作
...
# 6. 项目管道
ITEM_PIPELINES={
    '项目名.pipelines.类名':300
}
```
* 创建爬虫项目步骤
```
1. 新建项目:scrapy startporject 项目名称
2. cd 项目文件夹
3. 新建爬虫文件: scrapy gensipider 文件名 域名
4. 明确目标(items.py)
5. 写爬虫程序(文件名.py)
6. 管道文件(pipelines.py)
7. 全局配置(settings.py)
...
# 创建爬虫文件
scrapy genspider maoyan maoyan.com/board/4
```
...
3. 列表.extract_first() 或者 get():获取列表中第一个序列化的元素(字符串)
```
* pipelines.py中必须有一个函数叫process_item
```
def process_item(self,item,spider):
    return item
# 必须返回item,此返回值会传给下一个管道的函数继续处理
```
...
2. INFO     :一般信息
1. DEBUG    :调试信息
# 注意:只显示当前级别的日志和比当前级别日志更严重的信息
```
* 管道文件使用
```
1. 在爬虫文件中为items.py中类型做实例化,用爬下来的数据给对象赋值
    from ..item import MaoyanItem
    item = MaoyanItem()
2. 管道文件(pipelines.py)
3. 开启管道(settings.py)
...
## 今日作业
```
1. scrapy框架有哪几大组件?以及各组件之间是如何工作的?
2. 腾讯招聘尝试改写为scrapy
