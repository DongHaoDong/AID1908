...
线性回归、岭回归、多项式回归、svm.SVR()、决策树、正向激励、随机森林...     评估指标：r2_score(),平均绝对值误差

分类：

逻辑回归、朴素贝叶斯、决策树、随机森林、svm.SVC()。评估方式：混淆矩阵、分类报告。评估指标：查准率，召回率，f1得分

模型选择(针对分类模型)：交叉验证、验证曲线、学习曲线、数据集划分、网格搜索

模型评估指标：查准率、召回率、f1得分；可以使用混淆矩阵、分类报告。

完成一个具体的业务需求流程：

1. 认真理解业务需求，整理所需要的样本数据。
2. 不涉及算法，想清楚是分类业务还是回归业务。
3. 想清楚人类在针对当前业务做预测时会怎么做。
4. 找到最合适的模型，搞事情。
