...
print("====================================")
# 第三种设置dtype的方式
c = np.array(data,dtype={'names':['name','scores','ages'],'formats':['U2','3int32','int32']})
...
d = np.array(data,dtype={'names':('U3',0),
                    'scores':('3int32',16),
                    'ages':('int32',28)})
print(d['names'],d[0]['scores'],d.itemsize)
print("====================================")
# 第五种设置dtype的方式
...
# 测试日期类型数组
f = np.array(['2011','2012-01-01','2013-01-01 0:01:01','2011-02-01'])
f = f.astype('M8[D]')
f = f.astype('int32')
print(f[3]-f[0])
print("===================================")
...
|3i4|释义|
|---|----|
|3i4|打断字节序,3个元素的一维数组，每个元素都是整型，每个整型元素占4个字节|
|<(2,3)u8|小端字节序，6个元素2行3列的二维数组，每个元素都是无符号整型，每个无符号整型元素占8个字节|
|U7|包含7个字符串的Unicode字符串，每个字符占4个字节，采用默认字节序|  
**ndarray数组对象维度的操作**  
...
i = np.dstack((a,b))
# 水平方向(3维)完成拆分操作，生成两个数组
k,l=np.dsplit(i,2)
```
多维数组组合与拆分的相关函数
```
# 若通过axis作为关键字参数指定组合的方向，取值如下:
# 若待组合的数组都是二维数组
# 0:垂直方向
# 1:水平方向
# 若待组合的数组都是三维数组
...
b = np.arange(9,17) # [9,10,11,12,1,3,14,15,16]
# 把两个数组组合在一起成两行
c = np.row_stack((a,b))
print(c)
# 把两个数组组合在一起成两列
