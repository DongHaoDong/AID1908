...
1. number数值类型
    * 整数
        ...
        2. 八进制表示  
            ...
            ```
        ```html
        使用：整数可以采用不同进制表示，在控制台输出是一律会按照十进制输出
        ```
    * 小数
        1. 小数点表示
            ```html
        ...
        2. 科学计数法
            ...
            ```
2. string字符串类型  
    * 字符串：由一个或多个字符组成，使用""或''表示，每位字符都有对应的Unicode编码
    ```html
    var s = "100";
    var s1 = "张三";
    ```
3. boolean布尔类型  
    * 只有真和假两个值，布尔值与number值可以相互转换，true为1，false为0
    ```html
    var isSave = true;
    var isChecked = false;
    ```
...
2. 转换number类型
    * Number(param)  
    参数为要进行数据类型转化的变量或值，返回转换后的结果：  
    如果转换成功，返回number值  
    如果转换失败，返回NaN,(Not a Number),只要数据中存在非number字符串，一律转换失败，返回NaN
    * parseInt(param)
    参数为要解析的数据
        作用：从数据中解析整数值  
        过程：  
...
4. 自增或自减运算符
    ...
    ```
    注意：
    * 自增或自减运算符在单独与变量结合时，放前和房后没啥区别
    * 如果自增或自减运算符与其他运算符结合使用，要区分前缀和后缀，做前缀，那就先++/--，再进行赋值或者其他运算，如果做后缀，就先结合其他运算符，再进行++/--
5. 关系运算符/比较运算符
    ```html
    >  <
    >= <=
    ...
    ===(全等) !==(不全等)
    ```
    1. 关系运算符用来判断表达式之间的关系，结果永远是布尔值true/false
    2. 使用
        * 字符串与字符串之间的比较  
          依次比较每位字符串的Unicode码，只要某位字符比较出结果，就返回最终结果
        * 其他情况  
          一律将操作转换为number进行数值比较，如果某一操作数无法转换number,则变成NaN参与比较运算,结果永远是false
    3. 相等与全等
        * 相等：不考虑数据数据类型，只做值的比较(包含自动类型转换)
        * 全等：不会进行数据类型转换，要求数据类型一致并且值相等才会判断全等
6. 逻辑运算符
    1. &&逻辑与    
