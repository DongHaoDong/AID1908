...
3.url()函数
    ...
    * 语法
        ...
        * 参数
            ...
            2. views:指定路由所对应的视图处理函数的名称
            3. name:为地址起别名，在模板中地址反向 解析时使用
...
* 示例
    * 路由配置文件
        ...
        urlpatterns = [
            ...
            re_path(r'admin/',admin.size.urls),
...
* 练习
    ...
    * 如：
        ...
        显示为:生日为:2015年12月11日
        输入网址:http://127.0.0.1:8000/birthday/2/25/2008
        显示为:生日为:2008年2月28日
* Pycharm社区版针对Django项目调试方法
    1. 添加自己调试配置
        * 选择Add Configuration...
    2. 点击+号添加一个自己的配置
        * 选择运行的项目的主模块位置manage.py
        * 添加runserver命令行参数
## HTTP协议的请求和响应
...
* HTTP1.0定义了3中请求方法:GET,POST和HEAD方法(最常用)
...
* HttpRequest对象
    ...
    * HttpRequest属性
        ...
        * encoding:字符串，表示提交的数据的编码方式
            * 如果为None则表示使用浏览器默认的设置，一般为'utf-8'
...
## HTTP响应
* 当浏览者访问一个网页时，浏览者的浏览器会向网页所在服务器发送出请求。当浏览器接收并显示网页前，此网页所在的服务器会返回一个包含HTTP状态码的信息头用以响应浏览器的请求
...
* 服务器端接收参数
    1. 判断request.method的值判断请求方式是否是get请求
        ```
    ...
    2. 获取客户端请求GET请求提交的数据
        ...
        2. 能够产生get请求方式的场合
            ...
            3. form表单中的method为get
                ...
                ```
> 一般查询字符串的大小会受到浏览器的限制(不建议超过2048字节)  
* 练习
