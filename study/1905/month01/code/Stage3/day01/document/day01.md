...
2. 无需安装和更新
    ...
    - 只需要一个浏览器执行即可
## 4. 开发前的准备
1. 运行环境：浏览器，设置chrome为默认的浏览器，作为网页文件的运行环境。  
2. 调试工具：浏览器自带的调试工具，使用快捷键F12或者右键检查打开  
3. 开发工具：不限，选用个人习惯的即可。(Sublime,VSCode,EditPlus,Webstrom等)
# HTML语法介绍
## 1. HTML介绍
超文本标记语言浏览器能够识别和解析的语言。通过标签的形式构建页面结构和填充内容
## 2. 标签
标签也称为标记元或元素，用于在网页中标记内容  
...
2. 分类：
    ...
    ```
3. 标签属性：
    - 标签属性写在开始标签中，使用空格与标签名隔开，用于设置当前标签的显示内容或者修饰显示效果。由属性名和属性值组成，属性值使用双引号表示。    
    例：
    ```html
    <img src="image/image01.jpg.jpg"/>
    ```
   - 同一个标签中可以添加若干组标签属性，使用空格间隔。例如
   ```html
      <img src="image/image01.jpg.jpg" width="200px" height="200px"/>
   ```
## 3. 使用
...
- 列表标签
    ...
    ```
    - 有序列表
    默认使用阿拉伯数字标识每条数据，可以使用start属性设置起始的值，   默认为1
    ```html
    <ol>
          <li>list item 列表项</li>
          <li>list item 列表项</li>
