...
class ListHelper:
    ...
    @staticmethod
    def get_min(list_target,function_handle):
        """
            通用的获取最小元素的方法
        :param list_target:需要搜索的列表
        :param function_handle:需要搜索的处理逻辑，函数类型
        ...
        for index in range(1, len(list_target)):
            if function_handle(min_value) > function_handle(list_target[index]):
                min_value = list_target[index]
        return min_value
    @staticmethod
    def order_by_descending(list_target,function_handle):
        """
            通用的降序排序方法
        :param list_target: 需要排序的数据
        :param function_handle: 需要排序的逻辑，函数类型
        ...
        for row in range(len(list_target) - 1):
            for col in range(row + 1, len(list_target)):
                if function_handle(list_target[row]) < function_handle(list_target[col]):
                    list_target[row], list_target[col] = list_target[col], list_target[row]
    @staticmethod
    def delete_all(list_target,function_condition):
        """
            根据指定条件删除所有元素
        :param list_target:需要操作的列表
        :param function_condition:删除的条件
        """
        for index in range(len(list_target) - 1, -1, -1):
            if function_condition(list_target[index]):
